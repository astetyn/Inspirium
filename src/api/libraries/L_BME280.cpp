...
bool BME280::init(void) {
  ...
  if(read8(BME280_ADDRESS, BME280_REG_CHIPID) != 0x60) {
      return false;
  }
  ...
  dig_H1 = read8(BME280_ADDRESS, BME280_REG_DIG_H1);
  dig_H2 = BME280Read16LE(BME280_REG_DIG_H2);
  dig_H3 = read8(BME280_ADDRESS, BME280_REG_DIG_H3);
  dig_H4 = (read8(BME280_ADDRESS, BME280_REG_DIG_H4) << 4) | (0x0F & read8(BME280_ADDRESS, BME280_REG_DIG_H4 + 1));
  dig_H5 = (read8(BME280_ADDRESS, BME280_REG_DIG_H5 + 1) << 4) | (0x0F & read8(BME280_ADDRESS, BME280_REG_DIG_H5) >> 4);
  dig_H6 = (int8_t)read8(BME280_ADDRESS, BME280_REG_DIG_H6);

  write8(BME280_ADDRESS, BME280_REG_CONTROLHUMID, 0x05);  //Choose 16X oversampling
  write8(BME280_ADDRESS, BME280_REG_CONTROL, 0xB7);  //Choose 16X oversampling
...
float BME280::getTemperature(void)
{
  ...
  int adc_T = BME280Read24(BME280_REG_TEMPDATA);
  adc_T >>= 4;
...
uint32_t BME280::getPressure(void)
{
  ...
  int adc_P = BME280Read24(BME280_REG_PRESSUREDATA);
  adc_P >>= 4;
...
uint32_t BME280::getHumidity(void)
{
  ...
  v_x1_u32r = (((((adc_H << 14) - (((int)dig_H4) << 20) - (((int)dig_H5) * v_x1_u32r)) + ((int)16384)) >> 15) * (((((((v_x1_u32r * ((int)dig_H6)) >> 10) * (((v_x1_u32r * ((int)dig_H3)) >> 11) + ((int)32768))) >> 10) + ((int)2097152)) * ((int)dig_H2) + 8192) >> 14));
